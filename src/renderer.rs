...
use crate::{camera::Camera, hittables::Hit, ray::Ray, vec3::Color};
...
impl Renderer {
    pub fn render(&self) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
        let style = ProgressStyle::with_template(
            "[Elapsed: {elapsed_precise}] Eta: {eta_precise} {bar:40.cyan/blue} {pos:>7}/{len:7}",
        )
        .unwrap();
        //    Rendering
        let pixels: Vec<u8> = (0..self.image_settings.height)
            ...
            .flat_map_iter(|(i, j)| {
                ...
                for _ in 0..self.image_settings.samples_per_pixel {
                    let u =
                        (i as f64 + rng.random::<f64>()) / ((self.image_settings.width - 1) as f64);
                    let v = (j as f64 + rng.random::<f64>())
                        / ((self.image_settings.height - 1) as f64);
        ...
        let pixels = pixels
            .chunks(4 * self.image_settings.width as usize) // times 4 due to R G B and A channels for each pixel
            .rev()
            .flatten()
            .copied()
            .collect();
        ...
        ImageBuffer::from_vec(
            self.image_settings.width,
            self.image_settings.height,
            pixels,
        )
        .unwrap()
    }
