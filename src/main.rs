...
use hittables::transformations::{RotateY, Translate};
...
use crate::renderer::{ImageSettings, Renderer, Scene};
...
fn random_scene() -> Box<dyn Hit> {
    ...
    let checker = Arc::new(Lambertian::new(CheckerTexture::new(
    ...
    for a in -11..=11 {
        for b in -11..=11 {
            let a_prime = a as f64 + (0.9 * rng.random::<f64>());
            let b_prime = b as f64 + (0.9 * rng.random::<f64>());
            let center = Point3::new(a_prime, 0.2, b_prime);

            if (center - Point3::new(4, 0.2, 0)).length() > 0.9 {
                let sphere: Sphere = match rng.random() {
                    x if (0.0..=0.8).contains(&x) => {
                        let albedo = SolidColor::new(Color::random() * Color::random());
                        let center2 = center + Point3::new(0, rng.random::<f64>() / 2.0, 0);
                        Sphere::new_moving(
                            center,
                            center2,
                            0.2,
                    ...
                    _ => Sphere::new_static(center, 0.2, Arc::new(Dialectric::new(1.5))),
                };
                world.push(sphere);
            }
        }
...
fn two_spheres() -> Box<dyn Hit> {
    ...
    objects.push(Sphere::new_static(
        Point3::new(0, -10, 0),
        10,
        Arc::new(Lambertian::new(checker1)),
    ));
    objects.push(Sphere::new_static(
        Point3::new(0, 10, 0),
        10,
...
fn two_perlin_spheres() -> Box<dyn Hit> {
    ...
    objects.push(Sphere::new_static(
        Point3::new(0, 2, 0),
        2,
        Arc::new(Lambertian::new(pertext1)),
    ));
...
fn cornell_smoke() -> Box<dyn Hit> {
    ...
    let cube1 = RotateY::new(cube1, 15.0);
    let cube1 = Translate::new(cube1, Vec3::new(265, 0, 295));
    objects.push(ConstantMedium::new_from_color(
        Box::new(cube1),
        0.01,
        Color::new(0, 0, 0),
    ));
...
fn final_scene_book2() -> Box<dyn Hit> {
    ...
    for i in 0..boxes_per_side {
        for j in 0..boxes_per_side {
            ...
            let x1 = x0 + w;
            let y1 = rng.random_range(1.0..=100.0);
            let z1 = z0 + w;
    ...
    objects.push(Sphere::new_moving(
        center1,
        center2,
        50,
    ...
    let emat = Arc::new(Lambertian::new(ImageTexture::new(
        "earthmap.jpg".to_string(),
    )));
    ...
    let ns = 1000;
    for _ in 0..ns {
        boxes2.push(Sphere::new_static(
            Point3::random_in_range(0, 165),
            10,
            white.clone(),
        ))
    }
    ...
    objects.push(Translate::new(
        RotateY::new(BVH::new(boxes2.list, 0.0, 1.0), 15.0),
...
fn main() {
    ...
    match SCENE {
        1 => {
            ...
            cam = Camera::new(look_from, look_at, Vec3::new(0, 1, 0), settings, 0, 1)
        }
        2 => {
            let look_from = Point3::new(13, 2, 3);
            let look_at = Point3::new(0, 0, 0);
